    ...
    async def on_ready():
        ...
        async def infrequent():
            ...
            for plex in checksList:
                ...
                # USERNAME_IGNORE = [username for username in USERNAME_IGNORE]
                ...
                #     # IGNORED_UIDS = [uid for uid, username in PLEXUSERS.items() if username in USERNAME_IGNORE]
                #     # IGNORED_UIDS.extend((int(ACCOUNT.id), 0))
                #     getIgnoredUserIDsSuccess = True
                # except Exception as e:
                #     print('exception while getting users from plex server: ' + plex[1] + ' ' + str(e))
                ...
                for user in TAUTULLI_USERS:
                    ...
                    if email is not None:
                        # region do they have an invite date and is it too old
                        with DB_CONNECTION:
                        ...
                        if TOTAL_SECONDS >= (REMOVE_LIMIT * 86400):
                            print("total seconds since last seen, longer than remove limit: " + str(USERNAME) + ", " + str(TOTAL_SECONDS))
                            if inviteLessThanRLOLD == False:
                                # dont remove them if they havent accepted the invite yet, and it hasnt been REMOVE_LIMIT # of days.
                                print("    invite date is not smaller than the remove limit. invitedate, todayminusremovelimit: " + str(invitedDate) + ", " + str(todayMinusRemoveLimit) )
                                print("        They do have an invited date. Invitedate: " + str(invitedDate))
                                if DRY_RUN == True:
                                    ...
                                    if getUserIDAndEmailDictionarySuccess:
                                        ...
                                        try:
                                            ...
                                            mismatch = True
                                            print("probably had an issue where tautulli was out of date with Plex. should restart tautullis. Exception: " + str(e2))
